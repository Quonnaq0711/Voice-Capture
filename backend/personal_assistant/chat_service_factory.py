...
different LLM providers (Ollama, vLLM, OpenAI, etc.) without code changes.
...
def get_chat_service() -> BaseChatService:
    ...
    - anthropic: Claude API (future implementation)
    ...
    Environment Variables:
        ...
        - PA_LLM_MODEL: Model name (default: gemma3:latest)
        - PA_LLM_BASE_URL: Ollama server URL
        - DASHBOARD_OLLAMA_URL: Alternative Ollama URL (fallback)
    ...
    with _instance_lock:
        ...
        logger.info("="*60)
        logger.info(f"Initializing Chat Service Factory (thread-safe)")
...
def _create_vllm_service() -> BaseChatService:
    ...
    try:
        ...
        logger.error("Failed to import ChatServiceVLLM. Make sure vLLM dependencies are installed.")
    ...
    logger.info("🚀 Creating vLLM Chat Service")
...
def _create_ollama_service() -> BaseChatService:
    ...
    """
    try:
        from backend.personal_assistant.chat_service import ChatService
    except ImportError as e:
        logger.error("Failed to import ChatService. Make sure Ollama dependencies are installed.")
    ...
    # Load configuration from environment
    model_name = os.getenv("PA_LLM_MODEL", "gemma3:latest")

    # Try PA_LLM_BASE_URL first, then fall back to DASHBOARD_OLLAMA_URL
    base_url = os.getenv(
        "PA_LLM_BASE_URL",
        os.getenv("DASHBOARD_OLLAMA_URL", "http://ollama-staging:11434")
    )
...
def reset_chat_service():
    ...
    logger.info("Chat service instance reset")
