...
class ChatServiceVLLM(BaseChatService):
        ...
        try:
            ...
            if hasattr(self, 'executor') and self.executor:
                try:
                    ...
                    logger.info("[vLLM] Cleaned up executor after initialization failure")
                except Exception as cleanup_error:
                    logger.warning(f"[vLLM] Error cleaning up executor: {cleanup_error}")
    ...
    def _count_message_tokens(self, messages: List) -> int:
        ...
        try:
            ...
            if self._tiktoken_encoder is None:
                logger.warning("[vLLM] tiktoken encoder not available, using character estimation")
        ...
        try:
            ...
            if cancellation_event:
                ...
                try:
                    ...
                    # Handle timeout
                    if not done:
                        logger.warning("[vLLM] Response generation timed out after 120 seconds")
        ...
        try:
            ...
            if cancellation_event:
                ...
                try:
                    ...
                    # Handle timeout
                    if not done:
                        logger.warning("[vLLM] Follow-up generation timed out after 120 seconds")
                        for pending_task in [task, wait_task]:
                            pending_task.cancel()
    ...
    def _generate_follow_up_sync(
        ...
        try:
            ...
            logger.info(f"[vLLM Follow-up] Input tokens: ~{input_token_count}, Dynamic max_tokens: {dynamic_max_tokens}")
        ...
        try:
            ...
            if cancellation_event:
                ...
                try:
                    ...
                    # Handle timeout
                    if not done:
                        logger.warning("[vLLM] Query optimization timed out after 120 seconds")
                        for pending_task in [task, wait_task]:
                            pending_task.cancel()
    ...
    def _optimize_sync_query(
        ...
        try:
            ...
            logger.info(f"[vLLM Optimization] Input tokens: ~{input_token_count}, Dynamic max_tokens: {dynamic_max_tokens}")
    ...
    def __del__(self):
        ...
        try:
            ...
            import sys
            print(f"[vLLM] Warning: Error during ChatServiceVLLM cleanup in destructor: {e}", file=sys.stderr)
