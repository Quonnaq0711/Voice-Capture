"""Unit tests for ChatServiceVLLM class."""

import pytest
import asyncio
import sys
import os
from unittest.mock import Mock, AsyncMock
from datetime import datetime

# Add backend directory to path
backend_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
sys.path.insert(0, backend_path)


class TestChatServiceVLLMInitialization:
    """Test cases for ChatServiceVLLM initialization."""

    def test_init_with_default_params(self, mock_chat_service_vllm):
        """Test initialization with default mock parameters."""
        # Assert
        assert mock_chat_service_vllm is not None
        assert mock_chat_service_vllm.model_name == "test-model"
        assert mock_chat_service_vllm.api_base == "http://test:8888/v1"
        assert mock_chat_service_vllm.temperature == 0.7
        assert mock_chat_service_vllm.max_tokens == 2048

    def test_has_required_attributes(self, mock_chat_service_vllm):
        """Test that service has all required attributes."""
        # Assert
        assert hasattr(mock_chat_service_vllm, 'model_name')
        assert hasattr(mock_chat_service_vllm, 'api_base')
        assert hasattr(mock_chat_service_vllm, 'temperature')
        assert hasattr(mock_chat_service_vllm, 'max_tokens')
        assert hasattr(mock_chat_service_vllm, 'max_history_turns')
        assert hasattr(mock_chat_service_vllm, 'max_model_len')
        assert hasattr(mock_chat_service_vllm, 'safety_margin')
        assert hasattr(mock_chat_service_vllm, 'store')


class TestThreadSafety:
    """Test thread safety features."""

    def test_create_llm_with_max_tokens(self, mock_chat_service_vllm):
        """Test creating request-specific LLM instances for thread safety."""
        # Arrange
        mock_llm1 = Mock()
        mock_llm1.max_tokens = 1500
        mock_llm2 = Mock()
        mock_llm2.max_tokens = 3000

        mock_chat_service_vllm._create_llm_with_max_tokens.side_effect = [mock_llm1, mock_llm2]

        # Act
        llm1 = mock_chat_service_vllm._create_llm_with_max_tokens(1500)
        llm2 = mock_chat_service_vllm._create_llm_with_max_tokens(3000)

        # Assert - Different instances with different max_tokens
        assert llm1 is not llm2
        assert llm1.max_tokens == 1500
        assert llm2.max_tokens == 3000
        assert mock_chat_service_vllm._create_llm_with_max_tokens.call_count == 2


class TestTokenCounting:
    """Test token counting functionality."""

    def test_count_message_tokens(self, mock_chat_service_vllm):
        """Test token counting for messages."""
        # Arrange
        messages = [
            {"role": "user", "content": "Hello, how are you?"},
            {"role": "assistant", "content": "I'm doing well, thank you!"}
        ]

        # Mock returns fixed token count
        mock_chat_service_vllm._count_message_tokens.return_value = 150

        # Act
        token_count = mock_chat_service_vllm._count_message_tokens(messages)

        # Assert
        assert token_count == 150
        mock_chat_service_vllm._count_message_tokens.assert_called_once_with(messages)

    def test_calculate_dynamic_max_tokens(self, mock_chat_service_vllm):
        """Test dynamic max_tokens calculation based on input."""
        # Arrange - Small input should allow more output
        mock_chat_service_vllm._calculate_dynamic_max_tokens.return_value = 2000

        # Act
        result1 = mock_chat_service_vllm._calculate_dynamic_max_tokens(100)

        # Assert
        assert result1 == 2000

        # Arrange - Large input should allow less output
        mock_chat_service_vllm._calculate_dynamic_max_tokens.return_value = 500

        # Act
        result2 = mock_chat_service_vllm._calculate_dynamic_max_tokens(3000)

        # Assert
        assert result2 == 500


class TestGenerateResponse:
    """Test response generation with timeout handling."""

    @pytest.mark.asyncio
    async def test_generate_response_success(self, mock_chat_service_vllm):
        """Test successful response generation."""
        # Arrange
        message = "Tell me about career opportunities"
        session_id = "test_session_123"
        expected_response = {
            "response": "Here are some career opportunities...",
            "follow_up_questions": ["Q1?", "Q2?", "Q3?"],
            "model": "test-model",
            "session_id": session_id,
            "status": "success"
        }

        mock_chat_service_vllm.generate_response.return_value = expected_response

        # Act
        response = await mock_chat_service_vllm.generate_response(
            user_message=message,
            session_id=session_id
        )

        # Assert
        assert response == expected_response
        mock_chat_service_vllm.generate_response.assert_called_once()

    @pytest.mark.asyncio
    async def test_generate_response_with_cancellation(self, mock_chat_service_vllm):
        """Test that response can be cancelled via cancellation_event."""
        # Arrange
        message = "Test message"
        session_id = "test_session"
        cancellation_event = asyncio.Event()

        mock_chat_service_vllm.generate_response.side_effect = asyncio.CancelledError()

        # Act & Assert
        with pytest.raises(asyncio.CancelledError):
            await mock_chat_service_vllm.generate_response(
                user_message=message,
                session_id=session_id,
                cancellation_event=cancellation_event
            )

    @pytest.mark.asyncio
    async def test_generate_response_timeout_handling(self, mock_chat_service_vllm):
        """Test that timeout is properly handled."""
        # Arrange
        message = "Long running query"
        session_id = "test_session"

        mock_chat_service_vllm.generate_response.side_effect = asyncio.TimeoutError(
            "Response generation timed out after 120 seconds"
        )

        # Act & Assert
        with pytest.raises(asyncio.TimeoutError) as exc_info:
            await mock_chat_service_vllm.generate_response(
                user_message=message,
                session_id=session_id
            )

        assert "timed out" in str(exc_info.value).lower()


class TestStreamingResponse:
    """Test streaming response generation."""

    @pytest.mark.asyncio
    async def test_generate_streaming_response(self, mock_chat_service_vllm):
        """Test streaming response generation."""
        # Arrange
        message = "Tell me a story"
        session_id = "test_session"

        async def mock_stream():
            yield {"type": "token", "content": "Once "}
            yield {"type": "token", "content": "upon "}
            yield {"type": "token", "content": "a time..."}
            yield {"type": "complete", "content": "Once upon a time...", "follow_up_questions": ["Q1?"]}

        # Configure mock to return a fresh async generator on each call
        mock_chat_service_vllm.generate_streaming_response.side_effect = lambda **kwargs: mock_stream()

        # Act
        chunks = []
        async for chunk in mock_chat_service_vllm.generate_streaming_response(
            user_message=message,
            session_id=session_id
        ):
            chunks.append(chunk)

        # Assert
        assert len(chunks) == 4
        assert chunks[0]["type"] == "token"
        assert chunks[-1]["type"] == "complete"


class TestFollowUpQuestions:
    """Test follow-up question generation."""

    @pytest.mark.asyncio
    async def test_generate_follow_up_questions(self, mock_chat_service_vllm):
        """Test follow-up question generation."""
        # Arrange
        user_message = "Tell me about AI"
        ai_response = "AI is fascinating..."
        session_id = "test_session"
        expected_questions = ["What is machine learning?", "How does AI work?", "What are AI applications?"]

        mock_chat_service_vllm.generate_follow_up_questions.return_value = expected_questions

        # Act
        questions = await mock_chat_service_vllm.generate_follow_up_questions(
            user_message=user_message,
            ai_response=ai_response,
            session_id=session_id
        )

        # Assert
        assert questions == expected_questions
        assert len(questions) == 3
        mock_chat_service_vllm.generate_follow_up_questions.assert_called_once()

    def test_generate_follow_up_sync_uses_per_request_llm(self, mock_chat_service_vllm):
        """Test that follow-up generation uses per-request LLM instance."""
        # Arrange
        prompt = "Generate follow-up questions"
        expected_questions = "1. Question 1?\n2. Question 2?\n3. Question 3?"

        mock_chat_service_vllm._generate_follow_up_sync.return_value = expected_questions

        # Act
        result = mock_chat_service_vllm._generate_follow_up_sync(prompt)

        # Assert
        assert result == expected_questions
        mock_chat_service_vllm._generate_follow_up_sync.assert_called_once_with(prompt)


class TestQueryOptimization:
    """Test query optimization functionality."""

    @pytest.mark.asyncio
    async def test_optimize_query_success(self, mock_chat_service_vllm):
        """Test successful query optimization."""
        # Arrange
        original_query = "tell me stuff about jobs"
        expected_result = {
            "original_query": original_query,
            "optimized_query": "What career opportunities and job options are available in my field?",
            "model": "test-model",
            "status": "success"
        }

        mock_chat_service_vllm.optimize_query.return_value = expected_result

        # Act
        result = await mock_chat_service_vllm.optimize_query(original_query)

        # Assert
        assert result == expected_result
        assert result["status"] == "success"
        mock_chat_service_vllm.optimize_query.assert_called_once()

    def test_optimize_sync_query_thread_safe(self, mock_chat_service_vllm):
        """Test that query optimization uses per-request LLM instance."""
        # Arrange
        query = "career advice"
        expected_optimized = "Professional career guidance and development recommendations"

        mock_chat_service_vllm._optimize_sync_query.return_value = expected_optimized

        # Act
        result = mock_chat_service_vllm._optimize_sync_query(query)

        # Assert
        assert result == expected_optimized
        mock_chat_service_vllm._optimize_sync_query.assert_called_once_with(query)


class TestSessionManagement:
    """Test session and conversation history management."""

    def test_get_session_history(self, mock_chat_service_vllm):
        """Test retrieving session history."""
        # Arrange
        session_id = "test_session_123"
        expected_history = Mock()

        mock_chat_service_vllm.get_session_history.return_value = expected_history

        # Act
        history = mock_chat_service_vllm.get_session_history(session_id)

        # Assert
        assert history == expected_history
        mock_chat_service_vllm.get_session_history.assert_called_once_with(session_id)

    @pytest.mark.asyncio
    async def test_get_conversation_history(self, mock_chat_service_vllm):
        """Test getting conversation history."""
        # Arrange
        session_id = "test_session"
        expected_history = [
            {"role": "user", "content": "Hello"},
            {"role": "assistant", "content": "Hi there!"}
        ]

        mock_chat_service_vllm.get_conversation_history.return_value = expected_history

        # Act
        history = await mock_chat_service_vllm.get_conversation_history(session_id)

        # Assert
        assert history == expected_history
        assert len(history) == 2
        mock_chat_service_vllm.get_conversation_history.assert_called_once_with(session_id)

    @pytest.mark.asyncio
    async def test_clear_memory(self, mock_chat_service_vllm):
        """Test clearing conversation memory."""
        # Arrange
        session_id = "test_session"
        mock_chat_service_vllm.clear_memory.return_value = True

        # Act
        result = await mock_chat_service_vllm.clear_memory(session_id)

        # Assert
        assert result is True
        mock_chat_service_vllm.clear_memory.assert_called_once_with(session_id)

    @pytest.mark.asyncio
    async def test_remove_messages_after_index(self, mock_chat_service_vllm):
        """Test removing messages after specific index."""
        # Arrange
        session_id = "test_session"
        message_index = 5
        mock_chat_service_vllm.remove_messages_after_index.return_value = True

        # Act
        result = await mock_chat_service_vllm.remove_messages_after_index(
            session_id=session_id,
            message_index=message_index
        )

        # Assert
        assert result is True
        mock_chat_service_vllm.remove_messages_after_index.assert_called_once()

    @pytest.mark.asyncio
    async def test_update_message_at_index(self, mock_chat_service_vllm):
        """Test updating a message at specific index."""
        # Arrange
        session_id = "test_session"
        message_index = 3
        new_content = "Updated message content"
        mock_chat_service_vllm.update_message_at_index.return_value = True

        # Act
        result = await mock_chat_service_vllm.update_message_at_index(
            session_id=session_id,
            message_index=message_index,
            new_content=new_content
        )

        # Assert
        assert result is True
        mock_chat_service_vllm.update_message_at_index.assert_called_once()


class TestHealthCheck:
    """Test health check functionality."""

    @pytest.mark.asyncio
    async def test_health_check_healthy(self, mock_chat_service_vllm):
        """Test health check when service is healthy."""
        # Arrange
        expected_result = {
            "status": "healthy",
            "model": "test-model",
            "base_url": "http://test:8888/v1"
        }

        mock_chat_service_vllm.health_check.return_value = expected_result

        # Act
        result = await mock_chat_service_vllm.health_check()

        # Assert
        assert result == expected_result
        assert result["status"] == "healthy"
        mock_chat_service_vllm.health_check.assert_called_once()

    @pytest.mark.asyncio
    async def test_health_check_unhealthy(self, mock_chat_service_vllm):
        """Test health check when service is unhealthy."""
        # Arrange
        expected_result = {
            "status": "unhealthy",
            "model": "test-model",
            "base_url": "http://test:8888/v1",
            "error": "Connection refused"
        }

        mock_chat_service_vllm.health_check.return_value = expected_result

        # Act
        result = await mock_chat_service_vllm.health_check()

        # Assert
        assert result == expected_result
        assert result["status"] == "unhealthy"
        assert "error" in result


class TestUserProfile:
    """Test user profile integration."""

    @pytest.mark.asyncio
    async def test_get_user_profile(self, mock_chat_service_vllm):
        """Test retrieving user profile."""
        # Arrange
        user_id = 123
        expected_profile = {
            "id": user_id,
            "current_job": "Software Engineer",
            "company": "Tech Corp",
            "skills": "Python, JavaScript, React"
        }

        mock_chat_service_vllm.get_user_profile.return_value = expected_profile

        # Act
        profile = await mock_chat_service_vllm.get_user_profile(user_id, db=None)

        # Assert
        assert profile == expected_profile
        assert profile["current_job"] == "Software Engineer"
        mock_chat_service_vllm.get_user_profile.assert_called_once()


class TestCleanup:
    """Test resource cleanup."""

    def test_cleanup_method_exists(self, mock_chat_service_vllm):
        """Test that cleanup method is available."""
        # Act
        mock_chat_service_vllm.cleanup()

        # Assert
        mock_chat_service_vllm.cleanup.assert_called_once()

    def test_cleanup_available_for_resource_management(self, mock_chat_service_vllm):
        """Test that cleanup is available for resource management."""
        # Assert - cleanup method should exist
        assert hasattr(mock_chat_service_vllm, 'cleanup')
        assert callable(mock_chat_service_vllm.cleanup)
