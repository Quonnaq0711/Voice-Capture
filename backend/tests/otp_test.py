...
#         result = await otp_service.otp_request(db, "test@example.com", OTPPurpose.REGISTRATION)
#         cprint(result)
...
async def test_otp_service():
    ...
    try:
        ...
        test_email = "shaquonna@sadaora.com"
        
        logger.info(f"Requesting OTP for {test_email}")
        result = await otp_service.otp_request(db, test_email, OTPPurpose.REGISTRATION)
        logger.info(f"OTP service result: {result}")
        
        if result:
            logger.info("✅ OTP service test successful")
        else:
            logger.info("❌ OTP service returned None")
            
    except Exception as e:
        logger.error(f"❌ OTP service test failed: {e}")
        import traceback
        traceback.print_exc()
    finally:
        db.close()
...
async def main():
    logger.info("🚀 Starting MailGun and OTP Test Suite")
    logger.info("=" * 50)
    ...
    if working_domain:
        ...
        logger.error("❌ Cannot proceed with tests - no working domain found")
