...
import { careerInsights as careerInsightsAPI } from '../../services/api';

export default function RecentInsights({ onNavigateToCareerInsights }) {
  ...
  const generatePersonalizedInsights = async () => {
    try {
      ...
      if (careerInsightsResponse?.status === 'success' && Array.isArray(careerInsightsResponse.insights)) {
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        // Map insight IDs to career insights sub-tabs
                        const insightIdMapping = {
                        ...
                        // If it's a career insight with a specific mapping, use the new navigation
                        if (insightIdMapping[insightId]) {
                          if (onNavigateToCareerInsights) {
                            onNavigateToCareerInsights(insightIdMapping[insightId]);
                          } else {
                            // Fallback to old navigation if callback not provided
                            navigate(`/agents/career?tab=${insightIdMapping[insightId]}`);
                          }
                        } else {
                          // For other insight types (no_career_analysis, career_growth, etc.)
                          if (insight.action.includes('Career')) {
                            if (onNavigateToCareerInsights) {
                              onNavigateToCareerInsights(); // Navigate to career insights without specific sub-tab
                            } else {
